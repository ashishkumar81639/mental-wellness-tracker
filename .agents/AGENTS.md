...
When a task involves system design, architecture, or feature scoping, read the `system-design` skill.

When a task involves backend code, APIs, database, agents, or RAG, read the `backend` skill.

When a task involves UI, components, styling, or design tokens, read the `ui` skill.

When a question is conversational (no file open) but architectural, design-related, or backend-shaped, explicitly read the relevant skill before answering.
Do not answer from memory of these rules - read them.
...
Applies to both backend and UI work.

1. Does it need to exist for the demo? If the judge never sees it, skip.
2. Already in this repo? Reuse the existing helper, component, route, or pattern.
3. Does the framework or platform give it for free? Use it.
4. Does an installed SDK give it for free? Use it. No custom wrappers.
...
For non-trivial features, walk the phases (see the `system-design` skill for the phase list).
After each phase, show 5-10 lines and wait for "go."
No code until phase 3.
Skipping a phase is fine; skipping silently is not - name what you skipped and the risk.
...
Grep every caller of the function you touch and fix once at the source.
